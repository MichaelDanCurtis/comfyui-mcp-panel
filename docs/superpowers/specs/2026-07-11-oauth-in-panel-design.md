...
| **Grok/xAI** | Loopback-PKCE (device endpoint also exists; v1 uses loopback per local-only decision) | OIDC discovery `auth.x.ai/.well-known/openid-configuration` → `/oauth2/authorize`, `/oauth2/token`, `/oauth2/device/code`, `/oauth2/revoke` | **UNKNOWN — spike (Task 1).** Public desktop client id, not printed in any source found; capture from a real grok-cli/Hermes login or register a client. | `~/.grok/auth.json` (community convention: `{access_token, refresh_token, expires_in}`) | S256 PKCE. API base `api.x.ai/v1`; may need `api:access` scope (not just `openid…`) for API calls — verify empirically. Endpoint allowlist: only send the bearer to `https://api.x.ai` / `*.x.ai`. |
