...
- **Grok client-id provenance (documented, not experimental):** xAI publishes no self-serve OAuth client registration (`registration_endpoint` absent from its OIDC discovery), so Grok in-panel OAuth uses the public Grok-CLI desktop client_id + the `grok-cli:access` scope — i.e. it presents to xAI as the Grok CLI. This is the SAME identity-reuse *mechanism* as Copilot's, but a DIFFERENT risk class: xAI has publicly sanctioned first-party OAuth sign-in for third-party clients (unlike GitHub's Copilot API terms, which forbid it), so Grok ships first-class (not experimental, not gated) with this provenance recorded. If xAI later offers self-serve registration, switch to an owned client_id.
